...
if __name__ == "__main__":
    ...
    def quotient(mu, f, oe, oeT, rpmin, m_petro, n_petro, r_petro, b_petro, k_petro, wp, woe):
        ...
        # # get subexpressions
        # sub_exprs, simplified_exprs = sym.cse((ux, uy, uz))#, symbols=sym.numbered_symbols("t"))
        # print(f"sub_exprs: {sub_exprs}")
    ...
    # print(f"Creating numba-compiled versions")
    # @cc.export("fun_lyapunov_control_numba", "f8(f8, f8, f8[:], f8[:], f8, f8, f8, f8, f8, f8, f8, f8[:])")
    # def fun_lyapunov_control_numba(mu, f, oe, oeT, rpmin, m_petro, n_petro, r_petro, b_petro, k_petro, wp, woe):
    #     return fun_lyapunov_control(mu, f, oe, oeT, rpmin, m_petro, n_petro, r_petro, b_petro, k_petro, wp, woe)
    # cc.compile()
    ...
    tstart = time.time()
    N_retry = int(1e4)
    print(f"Benchmarking fun_lyapunov_control with {N_retry} runs...")
    for _ in range(N_retry):
        us = fun_lyapunov_control(
            mu_n, f_n, oe_n, oeT_n, rpmin_n, m_petro_n, n_petro_n, r_petro_n, b_petro_n, k_petro_n, wp_n, woe_n
        )
    tend = time.time()
    print(f"Time: {(tend-tstart)/N_retry * 1e3} ms")
